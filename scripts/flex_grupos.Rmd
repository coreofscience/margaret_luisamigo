...
data <- articulos_unicos_2016_2020 |> 
  ...
  mutate(ZONA = str_remove(ZONA, " ; .*"),
         ZONA = if_else(is.na(ZONA), "NA", ZONA),
         ZONA = str_remove(ZONA, "; .*")) |> 
  group_by(categoria_revista) |> 
  arrange(categoria_revista)|> 
  count(ZONA, sort = T, name = "cantidad") |> 
  arrange(categoria_revista)
...
data <- articulos_unicos_2016_2020 |> 
  ...
  mutate(unidad_academica = str_remove(unidad_academica, " ; .*"),
         unidad_academica = if_else(is.na(unidad_academica), "NA", unidad_academica),
         unidad_academica = str_remove(unidad_academica, "; .*")) |> 
  group_by(categoria_revista) |> 
  arrange(categoria_revista)|> 
  count(unidad_academica, sort = T, name = "cantidad") |> 
  arrange(categoria_revista)
  datatable(data,options = list(pageLength = 50), colnames = c("Categoria revista", "Unidad academica", "Cantidad"))
