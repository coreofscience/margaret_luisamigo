...
nombre <- articulos_unicos_2016_2020 |> 
  select(revista, ISSN)

categ <- read_csv("https://docs.google.com/spreadsheets/d/1ALPh_lgq6OtxgbKXRUEFEmoWcY37gfsnyTszFXbHvWw/export?format=csv&gid=758989915") |> 
...
revistas <- full_join(revistas, categ)

revistas <- revistas |> full_join(nombre, by = "ISSN") |> 
  unique() |> 
  unite(revistas, c("revista.x","revista.y"),sep = ",", remove = TRUE)|> 
  mutate(revistas = str_remove(revistas, "NA,"),
         revistas = str_remove(revistas, ".*,"),
         categoria = ifelse(is.na(categoria),"Sin Vigencia en 2021",categoria),
         editora = ifelse(is.na(editora), "Sin información", editora))

rev <- articulos_unicos_2016_2020 |> select(revista, ISSN)

new <- full_join(rev, categ)

count <- new |> count(ISSN) |> 
  full_join(new, by="ISSN") |> 
  unique(ISSN)
...
dato2 <- articulos_unicos_2016_2020 |> count(categoria_revista) |>
  ...
  mutate(categoria_revista = 
           ifelse(is.na(categoria_revista),"N/A",categoria_revista))
p10 <- dato2 |> 
  plot_ly(labels= ~categoria_revista, values=~n, type = 'pie')
p10 <- p10 %>% layout(title = 'Categorias',
         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
p10
```
