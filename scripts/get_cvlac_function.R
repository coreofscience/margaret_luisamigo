...
get_cvlac <- function(url) {
  ...
  # Add code to extract "Obras o productos" data
  df_obras_productos_1 <- cvlac_df[[1]] %>%
    dplyr::filter(stringr::str_detect(X1, "Obras o productos"))

  if (length(df_obras_productos_1$X1) > 0) {
    df_obras_productos_2 <- df_obras_productos_1 %>%
      purrr::discard(~all(is.na(.) | . == "")) %>%
      dplyr::select(-X1, -X2) %>%
      dplyr::slice(1) %>%
      t() %>%
      data.frame() %>%
      dplyr::rename(obras_productos = 1) %>%
      dplyr::mutate(fecha_creacion = stringr::str_extract(obras_productos,
                                                          "\\d{4}(?=\\s*\\s)")) # Extract the date and add a new column 'fecha_creacion'
  } else {
    df_obras_productos_2 <- tibble(obras_productos = 0)
  }
  ...
  return(list(name = df_name,
              ...
              events = df_events_2,
              papers = df_papers_2,
              obras_productos = df_obras_productos_2))
