...
Reporte_proyectos_2021 <- read_excel(here("scripts",
                                          "Reporte_proyectos_2021.xls"))
```

# Informe 1

## Programas académicos con proyectos de investigación en ejecución
...
programas<-programas %>%  gt()

programas
```

## Facultades con proyectos de investigación en ejecución
```{r}
...
facultades <- facultades %>% filter(!is.na(Facultades)) %>% 
  count(Facultades,sort = TRUE,name = "Cantidad") %>% gt()

facultades

```
## Centros regionales con proyectos de investigación en ejecución
```{r}
...
regiones <- regiones %>% filter(!is.na(Centros_Regionales)) %>% 
  count(Centros_Regionales,sort = TRUE,name = "Cantidad") %>% gt()
...
## Total proyectos convocatoria externa y Total proyectos convocatoria interna
```{r}
...
convocatorias <- convocatorias %>% filter(!is.na(Convocatorias)) %>% 
  count(Convocatorias, sort = TRUE, name = "Cantidad") %>% gt()
...
total_financiacion <- merge(presupuesto_externo,presupuesto_interno) %>%  gt()
